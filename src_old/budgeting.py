...
def apply_50_30_20_rule(
    ...
    try:
        ...
        print(
            f"Estimated income: ${income:.2f} ({income_source}, estimated ${estimated_income or 0:.2f} {'too low' if estimated_income else 'not found'})"
        )
        ...
        food_total = food_df[food_df["amount"] > 0][
            "amount"
        ].sum()  # Exclude negative amounts
        ...
        print(
            f"Savings/Debt: ${savings_debt_spending:.2f} ({savings_debt_percentage:.1f}%, {report['savings_debt']['status'].lower()} 20%)"
        )
