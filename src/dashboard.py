...
if os.path.exists(transactions_path):
    with open(transactions_path, "r") as f:
        st.session_state.transactions_data = json.load(f) if os.path.getsize(transactions_path) > 0 else []
else:
        ...
        with st.form(key="create_savings_plan_form"):
            ...
            if st.form_submit_button("Save Plan", key="save_plan_button", help="Save the new savings plan"):
                if not plan_name.strip() or plan_goal < 0:
                    ...
                    new_transaction = {"txnId": txn_id, "date": datetime.now().isoformat(), "amount": plan_goal, "category": "Savings"}
        ...
        with col_2:
            ...
            if st.button("Add to Plan", key="add_to_plan_button"):
                if amount <= st.session_state.balance and amount > 0:
                    ...
                    # Generate unique txnId and add transaction
                    txn_id = f"txn_{int(datetime.now().timestamp())}"
                    new_transaction = {"txnId": txn_id, "date": datetime.now().isoformat(), "amount": amount, "category": "Savings"}
                    st.session_state.transactions_data.append(new_transaction)
                    with open(transactions_path, "w") as f:
                        json.dump(st.session_state.transactions_data, f, indent=4)
