...
st.markdown(
    '<div class="header-container"><h1>FinAgent Financial Dashboard Test</h1></div>',
    unsafe_allow_html=True,
)

# Budget Distribution and Spending Analysis in two columns
st.markdown(
    "<h2 class='section-header'>Financial Overview</h2>", unsafe_allow_html=True
)
...
with col1:
    ...
    if os.path.exists(budget_path):
        with open(budget_path, "r") as f:
            budget = json.load(f)
        st.markdown(
            "<h3 class='section-header'>Budget Distribution</h3>",
            unsafe_allow_html=True,
        )
        fig = px.pie(
            values=[
...
with col2:
    ...
    if os.path.exists(db_path):
        ...
        report = pd.read_sql_query(
            "SELECT * FROM monthly_reports WHERE month = '2025-06'", conn
        )
        conn.close()
        if not report.empty:
            st.markdown(
                "<h3 class='section-header'>Spending Analysis</h3>",
                unsafe_allow_html=True,
            )
...
st.markdown(
    "<h2 class='section-header'>Guidance and Queries</h2>", unsafe_allow_html=True
)
...
with col3:
    ...
    if os.path.exists(advice_path):
        with open(advice_path, "r") as f:
            advice = json.load(f)
        st.markdown(
            "<h3 class='section-header'>Financial Advice</h3>", unsafe_allow_html=True
        )
...
with col4:
    if os.path.exists(advice_path):
        with open(advice_path, "r") as f:
            advice = json.load(f)
        st.markdown(
            "<h3 class='section-header'>Query Responses</h3>", unsafe_allow_html=True
        )
        for qa in advice["qa_responses"]:
            st.write(f"**Query**: {qa['query']}")
            st.write(f"**Answer**: {qa['answer']}")
        query = st.text_input(
            "Ask a financial question (e.g., 'What’s my best savings plan?')",
            key="query_input",
        )
        if query:
            prompt = (
