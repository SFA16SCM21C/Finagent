...
def generate_advice_and_qa(
    ...
    try:
        ...
        if "Low Savings/Debt" in risks:
            advice.append(
                f"Increase debt payments by ${income*0.025:.2f}/month to reduce interest costs."
            )
        if "High Wants" in risks:
            advice.append(
                f"Reduce Wants spending by ${report['wants_amount']*0.4:.2f} to align with 30% target."
            )
        ...
        for qa in qa_responses:
            if (
                "Food" in qa["query"]
                and abs(float(qa["answer"].split("$")[1].split(" ")[0]) - 16.33) > 0.01
            ):
                print(
                    f"Warning: Food spending answer ({qa['answer']}) does not match expected $16.33."
                )
