...
def calculate_debt_payoff(balance, interest_rate, minimum_payment, extra_payment=0):
    ...
    print(f"Debt Payoff Inputs: Balance=${balance:.2f}, Interest Rate={interest_rate:.1f}%, Minimum Payment=${minimum_payment:.2f}, Extra Payment=${extra_payment:.2f}")
...
def analyze_spending(input_path="data/transactions_cleaned.json", raw_input_path="data/transactions.json", db_path="data/finagent.db", income=4000.0):
    ...
    try:
        ...
        spending = df_month[df_month['amount'] > 0].groupby('category')['amount'].sum().to_dict()
        food_total = spending.get('Food', 0.0)
        needs_food = food_total * 0.5
        wants_food = food_total * 0.5

        needs_spending = sum(spending.get(cat, 0.0) for cat in needs_categories) + needs_food
        wants_spending = sum(spending.get(cat, 0.0) for cat in wants_categories) + wants_food
        savings_debt_spending = sum(spending.get(cat, 0.0) for cat in savings_debt_categories)
        ...
        if savings_debt_percentage < 10:
            risks.append(f"Low Savings/Debt spending: {savings_debt_percentage:.1f}% (below 10%)")
        ...
        raw_df_month = raw_df[(raw_df['date'].dt.year == 2025) & (raw_df['date'].dt.month == 6)]
        ...
        if not loan_payments.empty:
            ...
            months, total_interest = calculate_debt_payoff(
                debt['balance'], debt['interest_rate'], debt['minimum_payment'], extra_payment
            )
