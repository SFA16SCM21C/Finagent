...
def generate_sample_transactions(count=600):
    ...
    for week_date in income_weeks[:int(count * 0.04)]:  # Approx 4% of transactions as income
        transaction = {
            "account_id": f"acc{random.randint(1, 3)}",
    ...
    print(f"Generated and saved {len(transactions)} sample transactions to data/transactions.json")
