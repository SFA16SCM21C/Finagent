...
def estimate_income(raw_input_path="data/transactions.json", month=None):
    ...
    try:
        ...
        income = -income_df['amount'].sum()  # Sum of negative amounts as positive income
        return income if income > 0 else None
    except Exception as e:
        print(f"Error estimating income: {e}")
        return None

def apply_50_30_20_rule(clean_input_path="data/transactions_cleaned.json", output_path="data/budget_report.json", default_income=4000.0, custom_savings_goal=None):
    ...
    try:
        ...
        for month in unique_months:
            ...
            # Allocate spending
            needs_spending = df_month[df_month['category'].isin(needs_categories) & (df_month['amount'] > 0)]['amount'].sum()
            wants_spending = df_month[df_month['category'].isin(wants_categories) & (df_month['amount'] > 0)]['amount'].sum()
            ...
            savings_debt_percentage = (savings_debt_spending / income) * 100 if income > 0 else 0
            ...
            print(f"Savings/Debt: €{savings_debt_spending:.2f} ({savings_debt_percentage:.1f}%, {report['savings_debt']['status'].lower()} 20%, Goal: €{custom_savings_goal:.2f})")
